...
io.sockets.on('connection', function(socket){
  ...
  socket.on('reset', function(graph_title){
    yes = 1;
    no = 1;
  ...
  socket.on('go_to_slide', function(current_section, current_slide, question){
    slide = current_slide;
    section = current_section;
    io.sockets.emit('change_slide', current_section, current_slide, question);
  });
  ...
  socket.on("send_text_input", function(text_sent){
    io.sockets.emit('get_text_input', text_sent);
  ...
  socket.on('initialize_graphs', function(graph_title){
    io.sockets.emit('update_graph', "#awesome-graph"+graph_title, 1, 1);
